...
EPLUS_RUN_PATH = THIS_DIR + "/../EnergyPlusV9-5-0/energyplus"

def convert_json_idf(model_path):
...
def run_eplus_model(idf_path, output_dir,
	...
	print(eplus_run_script)
	logger.info(f'EnergyPlus run script: {eplus_run_script}')
...
def _log_subprocess_err(out, logger):
	for line in iter(out.readline, b''):
		logger.error(line.decode(errors='replace'))
